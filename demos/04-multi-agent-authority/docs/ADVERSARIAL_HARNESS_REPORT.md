# Adversarial Harness Report — Demo 04: Maxwell Multi-Agent Authority Demo
...
Every input under `examples/adversarial_inputs/` is expected to fail closed. That means no unauthorized `delegated_effect_record.json` should be created. A safe outcome should include `NO_DELEGATED_EFFECT_CREATED.txt`, a suppression notice, review/security/quarantine routing, or failed verification for tamper cases.
