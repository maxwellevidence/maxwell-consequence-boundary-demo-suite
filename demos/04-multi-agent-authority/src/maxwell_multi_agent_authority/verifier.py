...
def verify_run(run_dir: Path, write_report: bool = True) -> dict[str, Any]:
    ...
    effect_path = run_dir / "delegated_effect_record.json"
    no_effect_path = run_dir / "NO_DELEGATED_EFFECT_CREATED.txt"
    review_path = run_dir / "review_ticket.json"
    suppression_path = run_dir / "suppression_notice.json"
    ...
    if effect_permitted:
        add("delegated_effect_record_required", effect_path.exists(), "effect_permitted=true")
        add("no_effect_marker_absent", not no_effect_path.exists())
        add("review_ticket_absent", not review_path.exists())
        add("suppression_notice_absent", not suppression_path.exists())
    else:
        add("delegated_effect_record_absent", not effect_path.exists(), "effect_permitted=false")
        add("no_effect_marker_exists", no_effect_path.exists(), "effect_permitted=false")
        if outcome == "hold":
            add("review_ticket_required", review_path.exists(), "outcome=hold")
            add("suppression_notice_absent", not suppression_path.exists(), "outcome=hold")
        elif outcome == "block":
            add("suppression_notice_required", suppression_path.exists(), "outcome=block")
            add("review_ticket_absent", not review_path.exists(), "outcome=block")
    ...
    if effect_path.exists():
        ...
        add(
            "delegated_effect_bound_to_decision_receipt",
            actual == expected,
            "" if actual == expected else "effect record is not bound to current decision receipt",
        )

    for artifact_name, artifact_path in {
        "review_ticket": review_path,
        "suppression_notice": suppression_path,
    }.items():
        if artifact_path.exists():
            artifact = read_json(artifact_path)
            expected = sha256_json(decision)
            actual = artifact.get("decision_receipt_sha256")
            add(
                f"{artifact_name}_bound_to_decision_receipt",
                actual == expected,
                "" if actual == expected else f"{artifact_name} is not bound to current decision receipt",
            )
...
def _finish(run_dir: Path, checks: list[dict[str, Any]], write_report: bool) -> dict[str, Any]:
    ...
    report = {
        "schema": "maxwell.multi_agent_authority.verification_report.v0.1",
        "verified_at": utc_now(),
