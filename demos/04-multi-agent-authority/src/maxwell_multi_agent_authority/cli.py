...
def _print_result_row(result: dict) -> None:
    print(
        ...
        f"delegated_effect_permitted={result['effect_permitted']}"
    )
...
def cmd_demo(args: argparse.Namespace) -> int:
    ...
    print(f"Processed {len(results)} multi-agent demo case(s).")
...
def cmd_verify(args: argparse.Namespace) -> int:
    ...
    for report in reports:
        print(
            ...
            f"delegated_effect_permitted={report['effect_permitted']}"
        )
    return 0


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Maxwell Multi-Agent Authority public demo CLI")
    sub = parser.add_subparsers(dest="command", required=True)

    demo = sub.add_parser("demo", help="Run all multi-agent demo inputs.")
    demo.add_argument("--inputs", default="examples/demo_inputs")
    demo.add_argument("--policy", default="policies/multi_agent_authority_policy.yml")
    ...
    run = sub.add_parser("run", help="Run a single multi-agent demo input.")
    run.add_argument("--input", required=True)
    run.add_argument("--policy", default="policies/multi_agent_authority_policy.yml")
