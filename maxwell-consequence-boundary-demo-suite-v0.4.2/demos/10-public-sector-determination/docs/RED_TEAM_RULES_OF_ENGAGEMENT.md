# Red-Team Rules of Engagement — Demo 10
...
An attacker wins this public demo if a hostile input causes `determination_effect_record.json` to be created without the evidence, authority, scope, policy, and required review/security/due-process conditions stated in `THREAT_MODEL.md` and `DEMO_SPEC.yml`.
...
- Public artifact boundaries: `determination_effect_record.json` versus `NO_DETERMINATION_EFFECT_CREATED.txt`.
