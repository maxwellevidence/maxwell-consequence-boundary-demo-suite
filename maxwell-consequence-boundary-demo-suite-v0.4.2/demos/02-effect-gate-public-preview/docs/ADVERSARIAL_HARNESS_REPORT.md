# Adversarial Harness Report — Demo 02: Maxwell Effect Gate Public Preview
...
Every input under `examples/adversarial_inputs/` is expected to fail closed. That means no unauthorized `effect_record.json` should be created. A safe outcome should include `absence of effect_record.json for non-allow outcomes`, a suppression notice, review/security/quarantine routing, or failed verification for tamper cases.
