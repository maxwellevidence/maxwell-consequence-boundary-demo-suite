# Adversarial Harness Report — Demo 05: Maxwell Incident Reconstruction Demo
...
Every input under `examples/adversarial_inputs/` is expected to fail closed. That means no unauthorized `effect_record.json` should be created. A safe outcome should include `NO_EFFECT_CREATED.txt`, a suppression notice, review/security/quarantine routing, or failed verification for tamper cases.
