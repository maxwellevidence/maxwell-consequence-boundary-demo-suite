...
def verify_run(run_dir: Path, write_report: bool = True) -> dict[str, Any]:
    ...
    effect_path = run_dir / "payment_effect_record.json"
    no_effect_path = run_dir / "NO_PAYMENT_EFFECT_CREATED.txt"
    review_ticket_path = run_dir / "review_ticket.json"
    suppression_notice_path = run_dir / "suppression_notice.json"
    ...
    if effect_permitted:
        add("payment_effect_record_required", effect_path.exists(), "effect_permitted=true")
        add("no_effect_marker_absent", not no_effect_path.exists())
        add("review_ticket_absent", not review_ticket_path.exists())
        add("suppression_notice_absent", not suppression_notice_path.exists())
    else:
        add("payment_effect_record_absent", not effect_path.exists(), "effect_permitted=false")
        add("no_effect_marker_exists", no_effect_path.exists(), "effect_permitted=false")
        if lifecycle_state == "PAYMENT_REVIEW_ROUTED":
            add("review_ticket_exists", review_ticket_path.exists(), "review-routed case")
            add("suppression_notice_absent", not suppression_notice_path.exists())
        if lifecycle_state == "PAYMENT_EFFECT_SUPPRESSED":
            add("suppression_notice_exists", suppression_notice_path.exists(), "suppressed case")
            add("review_ticket_absent", not review_ticket_path.exists())
    ...
    if effect_path.exists():
        ...
        add(
            "payment_effect_bound_to_decision_receipt",
            actual == expected,
            "" if actual == expected else "payment effect record is not bound to current decision receipt",
        )

    for path, check_name in [
        (review_ticket_path, "review_ticket_bound_to_decision_receipt"),
        (suppression_notice_path, "suppression_notice_bound_to_decision_receipt"),
    ]:
        if path.exists():
            record = read_json(path)
            expected = sha256_json(decision)
            actual = record.get("decision_receipt_sha256")
            add(check_name, actual == expected, "" if actual == expected else "record binding mismatch")
...
def _finish(run_dir: Path, checks: list[dict[str, Any]], write_report: bool) -> dict[str, Any]:
    ...
    report = {
        "schema": "maxwell.payment_demo.verification_report.v0.1",
        "verified_at": utc_now(),
