# Adversarial Harness Report — Demo 08: Maxwell Human Review / Escalation Demo
...
Every input under `examples/adversarial_inputs/` is expected to fail closed. That means no unauthorized `authorized_effect_record.json` should be created. A safe outcome should include `NO_AUTHORIZED_EFFECT_CREATED.txt`, a suppression notice, review/security/quarantine routing, or failed verification for tamper cases.
